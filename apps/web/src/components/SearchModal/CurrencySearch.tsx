...
import { Type, Token, Native } from "rcpswap/currency"
import React, {
  KeyboardEvent,
  RefObject,
  useCallback,
...
import { useSortedTokensByQuery } from "@rcpswap/wagmi"
...
export function CurrencySearch({
  ...
  const { data: searchToken, ...rest } = useTokenWithCache({
  ...
  const searchTokenIsAdded = searchToken ? hasToken(searchToken) : false
  ...
  const handleEnter = useCallback(
    (e: KeyboardEvent<HTMLInputElement>) => {
      if (e.key === "Enter") {
        const s = debouncedQuery.toLowerCase().trim()
        if (s === "eth" || s === "matic") {
          handleCurrencySelect(Native.onChain(chainId))
