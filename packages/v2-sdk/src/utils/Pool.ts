import { chainName } from 'rcpswap/chain'
import { Amount, Price, Token } from 'rcpswap/currency'
import {
  InsufficientInputAmountError,
  InsufficientReservesError,
} from 'rcpswap/dex'
import { BigintIsh, FIVE, ONE, ZERO, _997, _1000, sqrt } from 'rcpswap/math'
...
import { SerializedPool, PoolSchema } from './zod'
...
export class Pool {
  ...
  public static getAddress(tokenA: Token, tokenB: Token): string {
    if (!isRCPSwapChainId(tokenA.chainId)) {
      throw new Error(
        `ChainId Error: SushiSwapV2 is not available on ${chainName[tokenA.chainId]
        }`,
      )
    }

    return computePoolAddress({
      factoryAddress: RCPSWAP_FACTORY_ADDRESS[tokenA.chainId],
      tokenA,
      tokenB,
      initCodeHashManualOverride: RCPSWAP_INIT_CODE_HASH[tokenA.chainId]
    })
  }
  ...
  public serialize(): SerializedPool {
  ...
  public static deserialize(pair: SerializedPool): Pool {
    return new Pool(
