...
import { ConstantProductRPool, RToken } from '@rcpswap/tines'
import { add, getUnixTime } from 'date-fns'
import { getReservesAbi } from 'rcpswap/abi'
...
export abstract class UniswapV2BaseProvider extends LiquidityProvider {
  ...
  async initialize() {
    ...
    const results = await this.client
      ...
      .catch((e) => {
        console.warn(
          `${this.getLogPrefix()} - INIT: multicall failed, message: ${e.message
          }`,
        )
        return undefined
      })
    ...
    topPools.forEach((pool, i) => {
      ...
      if (res0 && res1) {
        ...
        console.error(
          `${this.getLogPrefix()} - ERROR INIT SYNC, Failed to fetch reserves for pool: ${pool.address
          }`,
        )
      }
    })
  ...
  async getOnDemandPools(
    ...
    const reserves = await this.client
      ...
      .catch((e) => {
        console.warn(
          `${this.getLogPrefix()} - UPDATE: on-demand pools multicall failed, message: ${e.message
          }`,
        )
        return undefined
      })
    ...
    if (this.isInitialized) {
      ...
      const [initialPoolsReserves, onDemandPoolsReserves] = await Promise.all([
        this.client
          ...
          .catch((e) => {
            console.warn(
              `${this.getLogPrefix()} - UPDATE: initPools multicall failed, message: ${e.message
              }`,
            )
            return undefined
          }),
  ...
  private updatePoolWithReserves(
    ...
    pools.forEach((poolCode, i) => {
      ...
      if (res0 && res1) {
        ...
        console.error(
          `${this.getLogPrefix()} - ERROR UPDATING RESERVES for a ${type} pool, Failed to fetch reserves for pool: ${pool.address
          }`,
        )
      }
    })
  ...
  startFetchPoolsData() {
    ...
    this.unwatchBlockNumber = this.client.watchBlockNumber({
      ...
      onError: (error) => {
        console.error(
          `${this.getLogPrefix()} - Error watching block number: ${error.message
          }`,
        )
      },
    })
