...
type EventArgs<Event> = Event extends TypedEvent<any, infer TArgsObject>
  ? TArgsObject
  : never;
  ...
  private getRequestArgs(
    ...
    receipt.logs.forEach((log) => {
      ...
      if (event.name === eventName) {
        args = event.args as unknown as EventArgs<
          SynthesizeRequestEvent | BurnRequestEvent
        >;
      }
    });
  ...
  private buildOtherSideFilter(receipt: TransactionReceipt): EventFilter {
    ...
    const event =
      this.direction === "burn"
        ...
        : this.symbiosis
            .synthesis(this.chainIdOut)
            .filters.SynthesizeCompleted();
