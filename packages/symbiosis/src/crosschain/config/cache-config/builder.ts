...
export class Builder {
  ...
  private async checkMetarouters() {
    ...
    for (let i = 0; i < chains.length; i++) {
      ...
      if (synthesis.address !== AddressZero) {
        synthesisMetaRouter = (
          await synthesis.callStatic.metaRouter()
        ).toLowerCase()
      }

      if (
        (portalMetaRouter &&
          metaRouterAddressFromConfig !== portalMetaRouter) ||
        (synthesisMetaRouter &&
          metaRouterAddressFromConfig !== synthesisMetaRouter)
      ) {
      ...
      const metaRouterGatewayAddressFromConfig =
        chain.metaRouterGateway.toLowerCase()
      const metaRouter = this.metaRouter(chain.id)
      const metaRouterGatewayAddressFromContract = (
        await metaRouter.callStatic.metaRouterGateway()
      ).toLowerCase()
  ...
  private async getThresholds(
    ...
    const calls = chainTokens.map((token) => ({
      ...
      callData: contract.interface.encodeFunctionData("tokenThreshold", [
        token.address,
      ]),
    }))
    const thresholdsResponse = await multicall.callStatic.tryAggregate(
      false,
      calls
    )
    return thresholdsResponse
      .map(([success, returnData], index) => {
        if (!success) {
          throw new Error(
            `Cannot get token threshold from portal on chain ${chain.id}`
          )
        }

        const threshold = contract.interface.decodeFunctionResult(
          "tokenThreshold",
          returnData
        )[0] as BigNumber
        return {
  ...
  private async buildThresholds(
    ...
    for (let i = 0; i < chains.length; i++) {
      ...
      if (chain.portal !== AddressZero) {
        const portal = this.portal(chain.id)
        const portalThresholds = await this.getThresholds(
          tokens,
          portal,
          "Portal",
          chain
      ...
      if (chain.synthesis !== AddressZero) {
        const synthesis = this.synthesis(chain.id)
        const synthesisThresholds = await this.getThresholds(
          tokens,
          synthesis,
          "Synthesis",
          chain
  ...
  private async buildOmniPools(tokens: TokenInfo[]): Promise<OmniPoolInfo[]> {
    ...
    for (let i = 0; i < this.config.omniPools.length; i++) {
      ...
      const indexes = [...Array(last).keys()].map((index: number) => ({
        ...
        callData: omniPool.interface.encodeFunctionData("indexToAsset", [
          index,
        ]),
      }))
      ...
      const poolTokens: OmniPoolToken[] = assets
        .map(([success, returnData], index) => {
          ...
          const asset = omniPool.interface.decodeFunctionResult(
            "indexToAsset",
            returnData
          )
          if (!asset["active"]) {
          ...
          if (!token) {
            console.log(
              `token ${asset["token"]} doesn't exist. skip index ${index}`
            )
            return
          }
          return {
  ...
  private async buildTokensList() {
    ...
    const chainsWithFabric = this.config.chains.filter(
      (chain) => chain.fabric !== AddressZero
    )
    for (let i = 0; i < chainsWithFabric.length; i++) {
      ...
      const synthResults = await multicall.callStatic.tryAggregate(
        false,
        synthCalls
      )
      const synths = synthResults.map(([success, returnData]) => {
        if (!success) {
          throw new Error(
            `Cannot get representations from fabric on chain ${chainWithFabric.id}`
          )
        }
