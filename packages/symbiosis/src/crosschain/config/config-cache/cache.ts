...
export class ConfigCache {
  ...
  public getOmniPoolByConfig(
    omniPoolConfig: OmniPoolConfig
  ): OmniPoolInfo | undefined {
    return this.data.omniPools.find((i) => {
      return (
        i.address.toLowerCase() === omniPoolConfig.address.toLowerCase() &&
        i.chainId === omniPoolConfig.chainId
      );
    });
  }
  ...
  public getOmniPoolTokenIndex(
    ...
    if (!omniPool) {
      throw new Error(
        `getOmniPoolIndex: cannot find omniPoolByConfig ${omniPoolConfig}`
      );
    }
  ...
  private getTokenInfoByToken(token: Token): TokenInfo {
    ...
    if (!found) {
      throw new Error(
        `Can't get tokenInfo by token ${token.address} ${token.chainId}`
      );
    }
