...
export class OmniTrade {
  ...
  public async init() {
    ...
    const indexIn = this.symbiosis.getOmniPoolTokenIndex(
      this.omniPoolConfig,
      this.tokenAmountIn.token
    );
    const indexOut = this.symbiosis.getOmniPoolTokenIndex(
      this.omniPoolConfig,
      this.tokenOut
    );

    const quote = await this.poolOracle.quoteFrom(
      indexIn,
      indexOut,
    ...
    if (!this.tokenAmountIn.equalTo(this.tokenAmountInMin)) {
      quoteMin = await this.poolOracle.quoteFrom(
        indexIn,
        indexOut,
        this.tokenAmountInMin.raw.toString()
      );
    }

    this.amountOut = new TokenAmount(
      this.tokenOut,
      quote.actualToAmount.toString()
    );

    const amountOutMinRaw = getMinAmount(
      this.slippage,
      quoteMin.actualToAmount.toString()
    );
