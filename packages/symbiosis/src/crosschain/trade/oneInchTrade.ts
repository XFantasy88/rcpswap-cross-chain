...
export class OneInchTrade implements SymbiosisTrade {
  ...
  public async init() {
    ...
    let json: any
    try {
      json = await this.symbiosis.makeOneInchRequest(
      ...
      throw new Error(
        `Cannot get 1inch swap on chain ${this.tokenAmountIn.token.chainId}: ${message}`
      )
    }
  ...
  static async getProtocols(
    symbiosis: Symbiosis,
    chainId: ChainId
  ): Promise<OneInchProtocols> {
    try {
      const json = await symbiosis.makeOneInchRequest(
        `${chainId}/liquidity-sources`
      )

      return json["protocols"].reduce(
        (acc: OneInchProtocols, protocol: Protocol) => {
