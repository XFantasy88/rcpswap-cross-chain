...
import {
  UniV3Factory__factory,
  UniV3Quoter__factory,
  UniV3Router__factory,
} from "../contracts";
...
export class UniV3Trade implements SymbiosisTrade {
  ...
  public async init() {
    ...
    this.amountOutMin = new TokenAmount(
      this.tokenOut,
      bestAmountOut.toString()
    );
    ...
    const slippageTolerance = new PercentUni(
      (this.slippage * 100).toFixed(0),
      "1000000"
    );
    ...
    const methodParameters = UniV3Trade.swapCallParameters(
      [trade],
      options,
      swap
    );
      ...
      for (const { route, inputAmount, outputAmount } of trade.swaps) {
        ...
        if (singleHop) {
          if (trade.tradeType === TradeType.EXACT_INPUT) {
            ...
            calldatas.push(
              UniV3Router__factory.createInterface().encodeFunctionData(
                "exactInputSingle",
