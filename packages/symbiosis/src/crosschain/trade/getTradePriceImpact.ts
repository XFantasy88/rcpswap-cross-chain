...
export async function getTradePriceImpact({
  ...
  const tokens = [
    wrappedToken(tokenAmountIn.token),
    wrappedToken(tokenAmountOut.token),
  ];
  ...
  const data = aggregated.map(
    ([success, returnData], i): BigNumber | undefined => {
      if (!success || returnData === "0x") return;
      const result = oracle.interface.decodeFunctionResult(
        "getRateToEth",
        returnData
      );
  ...
  const impactNumber =
    1 - Number.parseFloat(formatUnits(impact, multiplierPow));

  return new Percent(
    parseInt(`${impactNumber * JSBI.toNumber(BIPS_BASE)}`).toString(),
    BIPS_BASE
  );
}
