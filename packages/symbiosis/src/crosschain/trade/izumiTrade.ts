...
export class IzumiTrade implements SymbiosisTrade {
  ...
  public async init() {
    ...
    for (const baseToken of addresses.baseTokens) {
      if (
        baseToken.equals(wrappedTokenIn) ||
        baseToken.equals(wrappedTokenOut)
      ) {
        continue;
      }
      ...
      POSSIBLE_FEES.forEach((firstFee) => {
        POSSIBLE_FEES.forEach((secondFee) => {
          ...
          allRoutes.push({
            tokens: [tokenIn, baseToken, tokenOut],
            path,
            fees,
          });
        });
    ...
    const provider = this.symbiosis.getProvider(
    ...
    const calls = allRoutes.map(({ path }) => ({
      target: quoter,
      callData: quoterInterface.encodeFunctionData("swapAmount", [
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      const { acquire } = quoterInterface.decodeFunctionResult(
        "swapAmount",
        returnData
      );
    ...
    const initDecimalPriceEndByStart = getPriceDecimalEndByStart(
      bestRoute,
      pointsBefore
    );
    ...
    const bestOutputBNJS = new BNJS(bestOutput.toString()).dividedBy(
      10 ** this.tokenOut.decimals
    );
    const impactBNJS = spotPriceBNJS
      .minus(bestOutputBNJS)
      .div(bestOutputBNJS)
      .negated();

    this.priceImpact = new Percent(
      impactBNJS.times(BIPS_BASE.toString()).toFixed(0).toString(),
      BIPS_BASE
    );
    ...
    if (outputToken.isNative) {
      swapCalls.push(
        swapInterface.encodeFunctionData("unwrapWETH9", [
          "0",
          finalRecipientAddress,
        ])
      );
    ...
    const encodedCallData = abiCoder
      .encode(["uint128"], [this.tokenAmountIn.raw.toString()])
      .replace("0x", "");
  ...
  async getCurrentPoolPoints({ fees, tokens }: IzumiRoute) {
    ...
    const getPoolAddressesResults = await multicall.callStatic.tryAggregate(
      false,
      getPoolAddressesCalls
    );
    const poolsAddresses: string[] = getPoolAddressesResults.map(
      ([, returnData]) =>
        factoryInterface.decodeFunctionResult("pool", returnData)[0]
    );
    ...
    const points = statesResults.map(
      ([, returnData]) =>
        poolInterface.decodeFunctionResult("state", returnData)["currentPoint"]
    );
...
export const getTokenChainPath = (
  tokenChain: Token[],
  feeChain: number[]
): string => {
...
export const priceUndecimal2PriceDecimal = (
  ...
  return Number(
    priceUndecimalAByB.times(10 ** tokenA.decimals).div(10 ** tokenB.decimals)
  );
};
...
export const point2PriceDecimal = (
  ...
  if (point > 0) {
    priceDecimal = priceUndecimal2PriceDecimal(
      tokenX,
      tokenY,
      new BNJS(1.0001 ** point)
    );
...
export function getPriceDecimalEndByStart(
  route: IzumiRoute,
  points: number[]
): number {
  ...
  for (let i = 0; i < fees.length; i++) {
    const decimalPriceBackByFront = point2PriceDecimal(
      tokens[i + 1],
      tokens[i],
      points[i]
    );
