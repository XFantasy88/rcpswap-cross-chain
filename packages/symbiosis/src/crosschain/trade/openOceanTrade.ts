...
export class OpenOceanTrade implements SymbiosisTrade {
  ...
  public async init() {
    ...
    const { data, outAmount, to } = (json as any).data as OpenOceanQuote
  ...
  private async getPriceImpact({
    tokenAmountIn,
    tokenAmountOut,
  }: GetPriceImpactParams): Promise<Percent> {
    const getTokensResponse = await fetch(
      `${this.endpoint}/specify_tokenList`,
      {
        method: "POST",
        headers: {
    ...
    const inPrice = (tokens as any).data.find(
      (token: any) =>
        token.symbol.toLowerCase() === tokenAmountIn.token.symbol?.toLowerCase()
    )?.usd
    const outPrice = (tokens as any).data.find(
      (token: any) =>
        token.symbol.toLowerCase() ===
        tokenAmountOut.token.symbol?.toLowerCase()
    )?.usd
    ...
    const inBn = new BigNumber(tokenAmountIn.raw.toString()).multipliedBy(
      outDecimals
    )

    const inDecimals = new BigNumber(10).pow(tokenAmountIn.token.decimals)
    const outBn = new BigNumber(tokenAmountOut.raw.toString()).multipliedBy(
      inDecimals
    )
    ...
    return new Percent(
      impact.multipliedBy(BIPS_BASE.toString()).integerValue().toString(),
      BIPS_BASE
    )
  }
}
