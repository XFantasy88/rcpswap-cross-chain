...
export class Transit {
  ...
  protected static getDirection(
    chainIdIn: ChainId,
    chainIdOut: ChainId,
    omniPoolChainId?: ChainId
  ): BridgeDirection {
    const withManagerChain =
      chainIdIn === omniPoolChainId || chainIdOut === omniPoolChainId
    if (!withManagerChain) {
      return "v2"
    }
  ...
  protected buildCalldata(): string {
    ...
    return this.multicallRouter.interface.encodeFunctionData("multicall", [
      ...
      [
        this.trade.tokenAmountIn.token.address,
        this.trade.amountOut.token.address,
      ], // path
      [100], // offset
  ...
  protected getTradeAmountOut(
    tradeAmountOut: TokenAmount | undefined
  ): TokenAmount {
    if (!tradeAmountOut) {
  ...
  protected getFeeToken(): Token {
    const transitTokenOutChainId = this.isV2()
    ...
    // mint or v2
    const rep = this.symbiosis.getRepresentation(
      this.transitTokenIn,
      transitTokenOutChainId
    ) // sToken
    if (!rep) {
      throw new Error(
  ...
  protected getTradeTokenOut(): Token {
    ...
    const transitTokenInChainId = this.isV2()
      ? this.omniPoolConfig.chainId
      : this.amountIn.token.chainId
    const rep = this.symbiosis.getRepresentation(
      this.transitTokenOut,
      transitTokenInChainId
    ) // sUSDC
    if (!rep) {
      throw new Error(
