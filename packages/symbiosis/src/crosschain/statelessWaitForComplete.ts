...
async function getTxBridgeInfo(
  symbiosis: Symbiosis,
  chainId: ChainId,
  txId: string
): Promise<BridgeTxInfo | null> {
  ...
  const revertBurnRequestTopic =
    portal.interface.getEventTopic("RevertBurnRequest");
  const revertSynthesizeRequestTopic = synthesis.interface.getEventTopic(
    "RevertSynthesizeRequest"
  );

  const revertLog = receipt.logs.find((log) => {
    return !!log.topics.find(
      (topic) =>
        topic === revertBurnRequestTopic ||
        topic === revertSynthesizeRequestTopic
    );
  });
  if (revertLog) {
    const address = revertLog.address.toLowerCase();
    if (
      address !== portal.address.toLowerCase() &&
      address !== synthesis.address.toLowerCase()
    ) {
      throw new Error(
        `Transaction ${txId} is not a from synthesis or portal contract`
      );
    }

    const bridge = symbiosis.bridge(chainId);
    const oracleRequestTopic = bridge.interface.getEventTopic("OracleRequest");
    const oracleRequestLog = receipt.logs.find((log) =>
      log.topics.includes(oracleRequestTopic)
    );
    if (!oracleRequestLog) {
      throw new Error(
        `Transaction ${txId} have a OracleRequest call not from bridge contract`
      );
    }

    const { chainId: oracleRequestChainId } =
      bridge.interface.parseLog(oracleRequestLog).args;
    ...
    const { id: internalId, to: revertableAddress } =
      contract.interface.parseLog(revertLog).args;
  ...
  const synthesizeRequestTopic =
    portal.interface.getEventTopic("SynthesizeRequest");
  const burnRequestTopic = synthesis.interface.getEventTopic("BurnRequest");

  const log = receipt.logs.find((log) => {
    return !!log.topics.find(
      (topic) => topic === synthesizeRequestTopic || topic === burnRequestTopic
    );
  });
...
async function waitOtherSideTx(
  symbiosis: Symbiosis,
  bridgeInfo: BridgeTxInfo
): Promise<string> {
...
export async function statelessWaitForComplete(
  ...
  const bBridgeInfo = await getTxBridgeInfo(
    symbiosis,
    aBridgeInfo.externalChainId,
    bTxId
  );
  ...
  if (!bBridgeInfo) {
    return tryToFindThorChainDepositAndWait(
      symbiosis,
      aBridgeInfo.externalChainId,
      bTxId
    );
  ...
  return tryToFindThorChainDepositAndWait(
    symbiosis,
    bBridgeInfo.externalChainId,
    cTxId
  );
}

export async function tryToFindThorChainDepositAndWait(
  symbiosis: Symbiosis,
  chainId: ChainId,
  txHash: string
) {
...
export async function findThorChainDeposit(
  symbiosis: Symbiosis,
  chainId: ChainId,
  txHash: string
) {
