...
export class Zapping {
  ...
  private getTransactionRequest(fee: TokenAmount): TransactionRequest {
    ...
    const [relayRecipient, otherSideCalldata] =
      this.otherSideSynthCallData(fee);
  ...
  private otherSideSynthCallData(fee: TokenAmount): [string, string] {
    ...
    const tokenAmount = this.tradeA
      ? this.tradeA.amountOut
      : this.tokenAmountIn;
  ...
  private async getSynthToken(): Promise<Token> {
    const chainIdOut = this.omniPoolConfig.chainId;
    const rep = await this.symbiosis.getRepresentation(
      this.transitTokenIn,
      chainIdOut
    );
